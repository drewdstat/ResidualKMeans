...
getcenters<-function(Data, clusters, scale = T){
  ...
  rownames(centermat) <- unique(clusters)
  colnames(centermat) <- coln
  return(centermat)
}
...
NbClust_km <- function (data = NULL, diss = NULL, distance = "euclidean",
      ...
      for (bb in (1:B)) {
        ...
        if (bb == 1) {
          ...
          if (ClassNr == length(cl))
            ...
            #set.seed(1)
            pp2 <- kmeans(Xnew, ClassNr, 100, 100)$cluster
          }
  ...
  for (nc in min_nc:max_nc) {
    ...
    if (method == 8) {
      # set.seed(1)
      cl2 <- kmeans(jeu, nc + 1, 100, 100)$cluster
      # set.seed(1)
      clmax <- kmeans(jeu, max_nc, 100, 100)$cluster
      if (nc > 2) {
        # set.seed(1)
        cl1 <- kmeans(jeu, nc, 100, 100)$cluster
        clall <- cbind(cl1, cl2)
        # set.seed(1)
        cl0 <- kmeans(jeu, nc - 1, 100, 100)$cluster
        clall1 <- cbind(cl0, cl1, cl2)
      }
      if (nc == 2) {
        # set.seed(1)
  ...
  if (any(indice == 31) || (indice == 32)) {
    ...
    if (any(method == 1) || (method == 2) || (method ==
      ...
      # set.seed(1)
      partition <- kmeans(jeu, j, 100, 100)$cluster
    }
  }
  if (any(indice == 1) || (indice == 2) || (indice == 3) ||
    ...
    if (any(method == 1) || (method == 2) || (method ==
      ...
      # set.seed(1)
      partition <- kmeans(jeu, best.nc, 100, 100)$cluster
    }
  }
